...
class AuthService {
  ...
  static async validateUser(email, password) {
    try {
      ...
      try {
        ...
        const employee = result[0] || result;
      ...
      if (!user) {
        try {
          ...
          // Kiểm tra xem result có phải là mảng hay đối tượng
          const customer = result[0] || result;
      ...
      if (!user) {
        console.log('Đăng nhập thất bại - không tìm thấy người dùng hoặc mật khẩu không khớp');
  ...
  static async registerUser(userData) {
    try {
      ...
      const insertId = result.insertId ? Number(result.insertId) : 
                      (result[0] && result[0].insertId ? Number(result[0].insertId) : null);
...
static async isEmailAvailable(email) {
  ...
  static async getUserById(id, role) {
    try {
      ...
      if (role === 'customer') {
        query = 'SELECT customerID as id, fullName, email, phoneNumber, address, registrationDate FROM Customers WHERE customerID = ?';
        idField = 'customerID';
      } else {
        query = 'SELECT employeeID as id, fullName, email, phoneNumber, address, position, hireDate FROM Employees WHERE employeeID = ?';
        idField = 'employeeID';
      }
