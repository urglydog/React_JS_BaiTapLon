...
const ShoppingCardItem = () => {
  ...
  const backendURLVNPAY = import.meta.env.VITE_BACK_END_URL_VNPAY;
  ...
  const fetchVouchers = async () => {
    ...
    try {
      const response = await fetch(
        `${backendURL}/api/voucher/getAllVouchers`
      );
      if (!response.ok) {
  ...
  const handleVNPAYPayment = async (e) => {
    ...
    try {
      ...
      const vnpayURL = `${backendURLVNPAY}/order/create_payment_url?amount=${discountedTotal}&orderId=${orderID}`;
