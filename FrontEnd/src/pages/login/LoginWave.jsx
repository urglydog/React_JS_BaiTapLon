...
function AuthCard() {
  ...
  // Handle navigation to home with scroll to top
      ...
      <div className={`absolute top-0 left-1/2 w-1/2 h-full bg-gradient-to-r from-indigo-500 to-purple-700 text-white flex flex-col items-center justify-center transition-all duration-500 ease-in-out ${isActive ? "-translate-x-full rounded-r-[100px] rounded-l-none" : "translate-x-0 rounded-l-[100px] rounded-r-none"}`}>
        {isActive ? (
          <Panel 
            ...
            desc="Nhập thông tin để đăng nhập." 
            btnText="Đăng Ký" 
...
const Panel = ({ text, desc, btnText, onClick, backToHome }) => (
  <div className="text-center px-10">
    ...
    <button
      onClick={backToHome}
      className="border-2 border-white text-white px-6 py-2 rounded-lg uppercase font-bold mt-3 text-sm hover:bg-white hover:text-indigo-700 transition-colors ml-2"
    >
      Trở Về Trang Chủ
    </button>
  </div>
