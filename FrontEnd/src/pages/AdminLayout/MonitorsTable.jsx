...
// Form component for adding/editing monitors
const MonitorForm = ({ monitor = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: monitor?.productName || '',
    description: monitor?.description || '',
    price: monitor?.price || '',
    stockQuantity: monitor?.stockQuantity || '',
    categoryId: monitor?.categoryId || 1,
    isLoading: false,
    error: null,
  });
...
console.log(getProductById);
    ...
    const filteredMonitors = searchTerm.trim() === ''
      ...
      : monitors.filter((monitor) =>
          (monitor?.productName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
          (monitor?.description || '').toLowerCase().includes(searchTerm.toLowerCase())
        );
    ...
    // Deactivate/delete monitor handler
    const handleDeactivate = (monitor) => {
      setConfirmDialog({
        isOpen: true,
        monitor,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${monitor.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(monitor.id);
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentMonitor ? formState.currentMonitor.id : null;
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">Danh sách màn hình</h2>
        ...
        {!isLoading && monitors.length > 0 && filteredMonitors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="h-12 w-12 bg-gray-700 rounded-lg flex items-center justify-center">
                        <img
                          src={monitor.image || '/api/placeholder/48/48'}
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {monitor.stockQuantity !== undefined ? monitor.stockQuantity : 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDeactivate(monitor)}
