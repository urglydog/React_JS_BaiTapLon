...
const TabletForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    categoryID: tablet?.categoryID || '',
    image: tablet?.image || '', // Store image URL
    isLoading: false,
    error: null,
  });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentTablet?.id || formState.currentTablet?.productID;
          if (!productId) {
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">Danh sách Máy Tính Bảng</h2>
        ...
        {(loading || isLoading) && localTablets.length === 0 && (
        ...
        {!loading && !isLoading && localTablets.length > 0 && filteredTablets.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
        ...
        {localTablets.length > 0 && filteredTablets.length > 0 && (
            ...
            <table className={`min-w-full border ${currentTheme.table}`}>
              ...
              <tbody className={`divide-y ${theme === 'dark' ? 'divide-gray-700' : 'divide-gray-300'}`}>
                {filteredTablets.map((tablet, index) => (
                  <tr
                    key={tablet.id || tablet.productID || `tablet-${index}`}
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${currentTheme.secondaryText}`}>
                      {CATEGORY_BRAND_MAPPING[tablet.categoryID] || 'N/A'}
                    </td>
