...
const StorageForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    image: storage?.image || '', // Store image URL
    isLoading: false,
    error: null,
  });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentStorage?.id || formState.currentStorage?.productID;
          if (!productId) {
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">Danh sách thiết bị lưu trữ</h2>
        ...
        {isLoading && localStorageItems.length === 0 && (
        ...
        {localStorageItems.length > 0 && filteredStorage.length > 0 && (
