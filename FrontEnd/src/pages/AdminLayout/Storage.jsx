...
// Form component for adding/editing Storage
const StorageForm = ({ storage = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: storage?.productName || '',
    description: storage?.description || '',
    price: storage?.price || '',
    stockQuantity: storage?.stockQuantity || '',
    categoryId: storage?.categoryId || 4, // Giả sử categoryId cho Storage là 4
    isLoading: false,
    error: null,
  });
...
console.log(getProductById);
    ...
    if (activeMenu !== 'Storage') return null;
    ...
    // Edit storage handler
    const handleEdit = (storageItem) => {
      setFormState({
        isOpen: true,
        formType: 'edit',
        currentStorage: storageItem,
      });
    };

    // Deactivate/delete storage handler
    const handleDeactivate = (storageItem) => {
      setConfirmDialog({
        isOpen: true,
        storage: storageItem,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${storageItem.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(storageItem.id);
            setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
            setIsLoading(false);
          } catch (error) {
            console.error('Error deleting storage:', error);
            setError('Failed to delete storage: ' + (error.message || ''));
            setIsLoading(false);
          }
        },
      });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentStorage ? formState.currentStorage.id : null;
        ...
        console.error('Error saving storage:', error);
        setError('Failed to save storage: ' + (error.message || ''));
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">Danh sách thiết bị lưu trữ</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder="Tìm kiếm thiết bị lưu trữ..."
                value={searchTerm}
        ...
        {!isLoading && storage.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>Không tìm thấy thiết bị lưu trữ nào.</span>
          </div>
        )}

        {!isLoading && storage.length > 0 && filteredStorage.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
        ...
        {storage.length > 0 && filteredStorage.length > 0 && (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDeactivate(storageItem)}
