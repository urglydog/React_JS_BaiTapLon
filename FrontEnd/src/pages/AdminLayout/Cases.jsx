...
const CaseForm = ({ caseItem = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: caseItem?.productName || '',
    description: caseItem?.description || '',
    price: caseItem?.price || '',
    stockQuantity: caseItem?.stockQuantity || '',
    categoryId: caseItem?.categoryId || 5, // Giả sử categoryId cho Cases là 5
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Failed to save case',
      }));
    }
  };
...
console.log(getProductById);
    ...
    // Deactivate/delete case handler
    const handleDeactivate = (caseItem) => {
      setConfirmDialog({
        isOpen: true,
        caseItem,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${caseItem.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(caseItem.id);
            setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
            setIsLoading(false);
          } catch (error) {
            console.error('Error deleting case:', error);
            setError('Failed to delete case: ' + (error.message || ''));
            setIsLoading(false);
          }
        },
      });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentCase ? formState.currentCase.id : null;
        ...
        console.error('Error saving case:', error);
        setError('Failed to save case: ' + (error.message || ''));
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">Danh sách vỏ máy tính</h2>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="h-12 w-12 bg-gray-700 rounded-lg flex items-center justify-center">
                        <img
                          src={caseItem.image || '/api/placeholder/48/48'}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDeactivate(caseItem)}
