...
// Form component for adding/editing processors
const ProcessorForm = ({ processor = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: processor?.productName || '',
    description: processor?.description || '',
    price: processor?.price || '',
    stockQuantity: processor?.stockQuantity || '',
    categoryId: processor?.categoryId || 2, // Giả sử categoryId cho processors là 2
    isLoading: false,
    error: null,
  });
...
const ProcessorsTable = memo(
  ({
    ...
    if (activeMenu !== 'Processors') return null;
    ...
    // Filter processors based on search term
    const filteredProcessors = searchTerm.trim() === ''
      ? processors
      : processors.filter((processor) =>
          (processor?.productName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    // Deactivate/delete processor handler
    const handleDeactivate = (processor) => {
      setConfirmDialog({
        isOpen: true,
        processor,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${processor.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(processor.id);
            setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
            setIsLoading(false);
          } catch (error) {
            console.error('Error deleting processor:', error);
            setError('Failed to delete processor: ' + (error.message || ''));
            setIsLoading(false);
          }
        },
      });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentProcessor ? formState.currentProcessor.id : null;
        ...
        console.error('Error saving processor:', error);
        setError('Failed to save processor: ' + (error.message || ''));
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">Danh sách bộ vi xử lý</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder="Tìm kiếm bộ vi xử lý..."
                value={searchTerm}
        ...
        {!isLoading && processors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>Không tìm thấy bộ vi xử lý nào.</span>
          </div>
        )}

        {!isLoading && processors.length > 0 && filteredProcessors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>Không tìm thấy bộ vi xử lý phù hợp.</span>
          </div>
        )}

        {processors.length > 0 && filteredProcessors.length > 0 && (
            ...
            <table className={`min-w-full border ${currentTheme.table}`}>
              ...
              <tbody className={`divide-y ${theme === 'dark' ? 'divide-gray-700' : 'divide-gray-300'}`}>
                {filteredProcessors.map((processor, index) => (
                  <tr
                    key={processor.id || `processor-${index}`}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDeactivate(processor)}
        ...
        {formState.isOpen && (
          <ProcessorForm
            processor={formState.currentProcessor}
            onSave={handleSave}
