...
const ProcessorsTable = memo(
  ({
    ...
    const filteredProcessors = searchTerm.trim() === ''
      ? localProcessors
      : localProcessors.filter((processor) =>
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentProcessor?.id || formState.currentProcessor?.productID || formState.currentProcessor?.productName;
          if (!productId) {
        ...
        const errorMessage = error.message.includes('Product not found')
          ...
          : error.message || 'Không thể lưu bộ vi xử lý';
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        {isLoading && localProcessors.length === 0 && (
        ...
        {!isLoading && localProcessors.length > 0 && filteredProcessors.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
        ...
        {localProcessors.length > 0 && filteredProcessors.length > 0 && (
