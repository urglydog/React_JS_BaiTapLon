...
// Form component for adding/editing gaming gear
const GamingGearForm = ({ gamingGear = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: gamingGear?.productName || '',
    description: gamingGear?.description || '',
    price: gamingGear?.price || '',
    stockQuantity: gamingGear?.stockQuantity || '',
    categoryId: gamingGear?.categoryId || 1, // Adjust categoryId as needed for gaming gear
    isLoading: false,
    error: null,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        isLoading: false,
        error: error.message || 'Failed to save gaming gear',
      }));
    }
  };
...
const GamingGearTable = memo(
  ({
    ...
    updateProduct,
    deleteProduct,
    getProductById, // Not used in this implementation but included for consistency
  }) => {
...
console.log(getProductById);
    ...
    if (activeMenu !== 'GamingGear') return null;
    ...
    // Filter gaming gear based on search term
    const filteredGamingGear = searchTerm.trim() === ''
      ? gamingGear
      : gamingGear.filter((item) =>
    ...
    // Edit gaming gear handler
    const handleEdit = (gamingGearItem) => {
      setFormState({
        isOpen: true,
        formType: 'edit',
        currentGamingGear: gamingGearItem,
      });
    };

    // Deactivate/delete gaming gear handler
    const handleDeactivate = (gamingGearItem) => {
      setConfirmDialog({
        isOpen: true,
        gamingGear: gamingGearItem,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${gamingGearItem.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(gamingGearItem.id);
            setConfirmDialog((prev) => ({ ...prev, isOpen: false }));
            setIsLoading(false);
          } catch (error) {
            console.error('Error deleting gaming gear:', error);
            setError('Failed to delete gaming gear: ' + (error.message || ''));
            setIsLoading(false);
          }
        },
      });
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentGamingGear ? formState.currentGamingGear.id : null;
        ...
        console.error('Error saving gaming gear:', error);
        setError('Failed to save gaming gear: ' + (error.message || ''));
        setIsLoading(false);
        throw error;
      }
    };
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-center">Danh sách Gaming Gear</h2>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative w-64">
              <input
                type="text"
                placeholder="Tìm kiếm gaming gear..."
                value={searchTerm}
        ...
        {!isLoading && gamingGear.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            ...
            <span>Không tìm thấy gaming gear nào.</span>
          </div>
        )}

        {!isLoading && gamingGear.length > 0 && filteredGamingGear.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            <span>Không tìm thấy gaming gear phù hợp.</span>
          </div>
        )}

        {gamingGear.length > 0 && filteredGamingGear.length > 0 && (
...
export default GamingGearTable;