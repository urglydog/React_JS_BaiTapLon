...
const GamingGearForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    categoryID: gamingGear?.categoryID || '',
    image: gamingGear?.image || '',
    isLoading: false,
    error: null,
  });
...
// GamingGear component
const GamingGear = memo(
  ({
    activeMenu,
    gamingGear = [],
    theme = 'dark',
    ...
    const handleSave = async (productData) => {
      try {
        ...
        if (formState.formType === 'add') {
          ...
          const productId = formState.currentGamingGear?.id || formState.currentGamingGear?.productID;
          if (!productId) {
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">Danh sách Gaming Gear</h2>
        ...
        {(loading || isLoading) && localGamingGear.length === 0 && (
        ...
        {!loading && !isLoading && localGamingGear.length > 0 && filteredGamingGear.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
        ...
        {localGamingGear.length > 0 && filteredGamingGear.length > 0 && (
