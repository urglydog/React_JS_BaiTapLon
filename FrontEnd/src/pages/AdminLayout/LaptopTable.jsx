...
const LaptopForm = ({ computer = {}, onSave, onCancel, formTitle, theme }) => {
  const [formData, setFormData] = useState({
    productName: computer?.productName || '',
    description: computer?.description || '',
    price: computer?.price || '',
    stockQuantity: computer?.stockQuantity || '',
    categoryId: computer?.categoryId || 1,
    isLoading: false,
    error: null,
  });
...
const LaptopTable = memo(
  ({
    ...
    // Deactivate/delete laptop handler
    const handleDeactivate = (computer) => {
      setConfirmDialog({
        isOpen: true,
        computer,
        title: 'Xác nhận xóa',
        message: `Bạn có chắc chắn muốn xóa "${computer.productName}" không?`,
        confirmAction: async () => {
          try {
            setIsLoading(true);
            await deleteProduct(computer.id); // Use the passed deleteProduct function
    ...
    return (
      <div className={`p-6 ${currentTheme.container}`}>
        ...
        {!isLoading && computers.length > 0 && filteredComputers.length === 0 && (
          <div className={`flex justify-center items-center h-64 ${currentTheme.emptyState}`}>
            <ImageOff className="mr-2" size={24} />
            ...
            <table className={`min-w-full border ${currentTheme.table}`}>
              ...
              <tbody className={`divide-y ${theme === 'dark' ? 'divide-gray-700' : 'divide-gray-300'}`}>
                {filteredComputers.map((computer, index) => (
                  <tr
                    key={computer.id || `computer-${index}`}
